export default function ThreatRadar({ className = '', blips = 7 }) {
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      <svg viewBox="0 0 100 100" className="h-full w-full">
        ...
        <circle cx="50" cy="50" r="48" fill="url(#radarFill)" stroke="rgba(0,229,255,0.25)" strokeWidth="0.4" />
        {[16, 28, 40].map((r) => (
          <circle key={r} cx="50" cy="50" r={r} fill="none" stroke="rgba(0,229,255,0.18)" strokeWidth="0.35" />
        ))}
        <line x1="50" y1="2" x2="50" y2="98" stroke="rgba(0,229,255,0.12)" strokeWidth="0.3" />
        <line x1="2" y1="50" x2="98" y2="50" stroke="rgba(0,229,255,0.12)" strokeWidth="0.3" />
        <g style={{ transformOrigin: '50px 50px', animation: 'radar-spin 4.5s linear infinite' }}>
          <path d="M50 50 L50 4 A46 46 0 0 1 78 14 Z" fill="rgba(0,229,255,0.22)" />
        </g>
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="1.4" fill="#ff3b5c">
              <animate attributeName="opacity" values="1;0.2;1" dur="1.6s" begin={`${p.delay}s`} repeatCount="indefinite" />
            </circle>
            <circle cx={p.x} cy={p.y} r="4" fill="none" stroke="#ff3b5c" strokeOpacity="0.5">
